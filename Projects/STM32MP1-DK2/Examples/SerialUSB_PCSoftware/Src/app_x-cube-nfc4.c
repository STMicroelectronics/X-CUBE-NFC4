...
void VIRT_UART0_RxCpltCallback(VIRT_UART_HandleTypeDef *huart)
{
	static uint32_t msgLen = 0;

    /* copy received msg in a variable to sent it back to master processor in main infinite loop*/
    VirtUart0ChannelRxSize = huart->RxXferSize < MAX_BUFFER_SIZE? huart->RxXferSize : MAX_BUFFER_SIZE-1;
    memcpy(&VirtUart0ChannelBuffRx[msgLen], huart->pRxBuffPtr, VirtUart0ChannelRxSize);
    msgLen += VirtUart0ChannelRxSize;
    if(VirtUart0ChannelBuffRx[msgLen - 1] == TMSG_EOF)
    {
    	// end of frame detected
    	VirtUart0RxMsg = SET;
    	VirtUart0ChannelRxSize = msgLen;
    	msgLen = 0;
    }
}
...
SERIALCOM_StatusTypeDef SerialCom_ReceivedMSG( TMsg * const Msg )
{
 // static uint16_t DmaMsgIndex = 0;
  ...
  if (VirtUart0RxMsg)
  {
    ...
    payloadsize = VirtUart0ChannelBuffRx[SERIAL_COM_LENGTH];
    payloadsize = payloadsize << 8;
    payloadsize |= VirtUart0ChannelBuffRx[SERIAL_COM_LENGTH + 1];

    sizecount = VirtUart0ChannelRxSize -1;
    ...
    if( (SERIAL_COM_HEADERSIZE + payloadsize + SERIAL_COM_FOOTERSIZE) == sizecount )
    {
      ...
      for( cnt = 0; cnt < sizecount; cnt++ )
      {
        ...
        Msg->Data[cnt] = VirtUart0ChannelBuffRx[cnt];
        Msg->Len++;
      }
...
void SerialComUART_SendMsg( TMsg * const Msg )
{  
  memcpy( (void *)VirtUart0ChannelBuffRx, Msg->Data, Msg->Len );
  VirtUart0ChannelBuffRx[Msg->Len++] = '\n';
  VIRT_UART_Transmit(&huart0, VirtUart0ChannelBuffRx, Msg->Len);
}
