...
static void NDEF_FillVcardStruct( uint8_t* pPayload, uint32_t PayloadSize, char* pKeyWord, uint32_t SizeOfKeyWord, uint8_t* pString )
{
  ...
  *pString = 0;
  ...
  while( memcmp( pLastByteAdd, VCARD_JPEG, JPEG_STRING_SIZE ) && (pLastByteAdd < (pPayload + PayloadSize)) )
  {
    pLastByteAdd++;
  }
...
uint32_t NDEF_GetVcardLength( sVcardInfo *pVcardStruct)
{
  ...
  if((pVcardStruct->Picture != NULL))
  {
    PayloadSize += strlen(PICTURE_PHOTO);
    PayloadSize += strlen(VCARD_JPEG);
    PayloadSize++;
...
/************************************/
  ...
  if((pVcardStruct->Picture != NULL) && (pVcardStruct->PictureSize != 0))
  {
    ...
    PayloadSize += strlen(PICTURE_PHOTO);
    memcpy(&pNDEFMessage[PayloadSize],VCARD_JPEG,strlen(VCARD_JPEG));
    PayloadSize += strlen(VCARD_JPEG);
...
int NDEF_getVcardPicture( uint8_t* pPayload, uint32_t PayloadSize,  uint8_t* pPict )
{
  ...
  pSrcPict = pPayload;
  while( memcmp( pSrcPict, VCARD_JPEG, JPEG_STRING_SIZE ) && (pSrcPict < (pPayload + PayloadSize)) )
  {
    pSrcPict++;
  }
